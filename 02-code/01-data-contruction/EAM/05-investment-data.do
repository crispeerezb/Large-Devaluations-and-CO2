***** CODE FOR INVESTMENT BY FIRM FROM EAM *****
...
foreach year of numlist `start_year'/`end_year' {
	...
	* save data
	save "${dir_temp}/eam-investment/investment-data_`year'.dta", replace

}

* Finally we append data
cd "${dir_temp}/eam-investment"
use investment-data_2000.dta, clear 

foreach year of numlist 2001/2019 {
    append using investment-data_`year'.dta
}
...
* We save data
save "${output}/data-stata/eam/05-investment-data.dta", replace
