...
* Generate events for large devaluations
...
twoway (sc estimate time, mcolor(orange) mlcolor(orange) lcolor(orange) connect(direct)) ///
       ...
       yline(0) ///
       xtitle("Years Since Crises", size(medium)) ///
...
graph export "${results}\eora\event-study-for-all-crises\ES-BDC-Gross-Output.pdf", as(pdf) replace
