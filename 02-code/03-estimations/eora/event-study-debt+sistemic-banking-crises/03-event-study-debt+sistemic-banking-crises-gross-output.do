/*--------------------------------------------------------------*/
/*-------Event Study: Debt + Systemic Banking Crises------------*/
/*--------------------------------------------------------------*/
...
sort country year
gen grossoutput_ly=grossoutput_PPP[_n-1] if country==country[_n-1] & year==year[_n-1]+1
...
gen ln_gross_output_lag = log(grossoutput_ly)
...
* Generate events (for debt and banking crises)
...
* set y-title
global ytitle="Log Gross Output"
...
twoway (sc estimate time, mcolor(orange) mlcolor(orange) lcolor(orange) connect(direct)) ///
       ...
       yline(0) ///
       xtitle("Years Since Debt or Systemic Banking Crises", size(medium)) ///
       ytitle($ytitle, size(medium)) ///
       xlabel(-$monthsbefore(1)$monthsafter, labsize(medium)) ///
       legend(ring(0) position(4) order(1 "Point estimate" 2 "95% confidence interval")) ///
       graphregion(margin(large)) ///
       xsize(20) ysize(10)


graph export "${results}\eora\event-study-debt+banking-crises\ES-BD-Gross-Output.pdf", as(pdf) replace
