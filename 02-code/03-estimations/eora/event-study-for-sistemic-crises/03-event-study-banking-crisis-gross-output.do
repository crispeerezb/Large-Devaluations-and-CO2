...
* drop those parm that change graph
drop if parm == "lrgdpo"
drop if parm == "lemp"
drop if parm == "lrnna"
drop if parm == "_cons"
...
twoway (sc estimate time, mcolor(orange) mlcolor(orange) lcolor(orange) connect(direct)) ///
       ...
       yline(0) ///
       xtitle("Years since Systemic Banking Crisis", size(medium)) ///
...
*** save graph ***
graph export "${results}\eora\event-study-for-sistemic-crisis\ES-B-Gross-Output.pdf", as(pdf) replace
