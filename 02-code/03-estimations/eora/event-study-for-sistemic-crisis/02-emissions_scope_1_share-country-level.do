...
collapse (sum) co2_scope1 grossoutput, by(country country_code year)
...
keep country country_code year co2_scope1 grossoutput grossoutput_ly lrgdpo
...
global outcome1="ln_scope_1_rate" /*Set outcome scope1_eora or scope12_eora*/
global timevar="year" 
...
gen ln_scope_1_rate=ln(co2_scope1/grossoutput)
...
sort time
twoway (sc estimate time, mcolor(orange) mlcolor(orange) lcolor(orange) connect(direct)) (rcap min95 max95 time , lcolor(gs10) ), xline(-1, lpattern(dash) lcolor(black)) yline(0) xtitle("Years since Systemic Banking Crisis") ytitle("Scope 1 rates") xlabel(-$monthsbefore(1)$monthsafter) legend(ring(0) position(8) order(1 "Point estimate" 2 "95% confidence interval"))
