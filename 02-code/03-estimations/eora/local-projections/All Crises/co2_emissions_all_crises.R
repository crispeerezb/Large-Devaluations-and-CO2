...
df_crises <- read_dta(file.path(dir_output, "data-stata", "financial-crisis", "financial_crises.dta"))
...
df_eora <- df_eora %>%
  left_join(df_crises, by = c("country_code", "year"))
...
df_eora$crisis[is.na(df_eora$crisis)] <- 0
...
ggsave(file.path(dir_results, "emissions_all_crises.pdf"), width = 20, height = 10)
