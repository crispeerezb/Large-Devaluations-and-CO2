...
##########################################
# 4. Local projections for CO2 intensity #
##########################################
...
for (h in 0:max_h) {
  df_eora <- df_eora %>%
    group_by(country) %>%
    arrange(year) %>%
    mutate(!!paste0("log_co2_scope1_lead_", h) := lead(log_co2_scope1, n = h))
}
...
for (h in 0:max_h) {
  ...
  formula_lp <- as.formula(paste0(
    "log_co2_scope1_lead_", h, " ~ event + log(gross_output) + factor(country) + factor(year)"
  ))
...
coefficients_df <- data.frame(
  horizon = 0:max_h,
  beta = sapply(results, function(model) {
    coefs <- coef(model)
    if ("event" %in% names(coefs)) {
      return(coefs["event"])
    } else {
      return(NA)
    }
  }),
  se = sapply(results, function(model) {
    vcov_mat <- vcov(model)
    if ("event" %in% rownames(vcov_mat)) {
      return(sqrt(vcov_mat["event", "event"]))
    } else {
      return(NA)
    }
  })
...
# Remove rows with NA coefficients (if any)
coefficients_df <- coefficients_df %>% filter(!is.na(beta))
...
ggsave(file.path(dir_results, "co2_emissions_currency_crises.pdf"), width = 20, height = 10)
