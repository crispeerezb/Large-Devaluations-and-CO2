...
for (h in 0:max_h) {
  ...
  formula_lp <- as.formula(paste0(
    "log_gross_output_lead_", h, " ~ event + log(gross_output) + factor(country) + factor(year)"
  ))
...
ggsave(file.path(dir_results, "gross_output_currency_crises.pdf"), width = 20, height = 10)
