...
df_debt_crises <- read_dta(file.path(dir_output, "data-stata", "financial-crisis", "sovereign_debt_crisis.dta"))
...
df_eora <- df_eora %>%
  left_join(df_debt_crises, by = c("country_code", "year"))
...
df_eora$sovereign_debt_crisis_event[is.na(df_eora$sovereign_debt_crisis_event)] <- 0
...
ggsave(file.path(dir_results, "co2_emissions_debt_crises.pdf"), width = 20, height = 10)
