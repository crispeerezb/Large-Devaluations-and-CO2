...
for (h in 0:max_h) {
  ...
  formula_lp <- as.formula(paste0(
    "co2_intensity_lead_", h, " ~ event + log(gross_output) + factor(country) + factor(year)"
  ))
...
ggsave(file.path(dir_results, "co2_intensity_currency_crises.pdf"), width = 20, height = 10)
